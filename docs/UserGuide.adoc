...
[discrete]
=== Locating students by name or tag: `find` or `f`
...
// tag::seek[]
[discrete]
=== Seeking the Resident Assistant(s) of a student : `seek`
...
[discrete]
=== Show level of friendship feature : `show` or `sh`
...
// tag::sort[]
[discrete]
=== Sort RC4 Students List: `sort [INDEX_TYPE]'
...
[discrete]
=== Adding a reminder: `+reminder` or `+r` or `addreminder`
...
[discrete]
=== Deleting a reminder: `-reminder` or `-r` or `deletereminder`
...
//tag::goalfeatures[]
[discrete]
=== Adding a goal: `+goal` or `+g` or `addgoal`
...
[discrete]
=== Sorting goals: `sortgoal` or `sgoal`
...
[discrete]
=== Editing a goal: `~goal` or `~g` or `editgoal`
